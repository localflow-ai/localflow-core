...
- **External data access** — the strict vision focuses on where inference runs, but does not address how the AI can securely access external data sources (APIs, CRMs, ERPs). In practice, formulas often need to call outside services, and there is no built-in mechanism to do so in a controlled, auditable way.
