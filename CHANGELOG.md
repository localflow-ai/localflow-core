...
- **`LocalAssistant.addDocumentDataset(name, buffer, extractedText?, pageCount?, documentMetadata?)`** — format-generic replacement for `addPdfDataset()`. Non-PDF documents report `type: 'document'` from `getActiveDataset()`; PDFs keep reporting `'pdf'` so existing consumers are unaffected.
- **`getActiveDocumentBuffer()` / `getActiveDocumentExtractedText()` / `getActiveDocumentPageCount()` / `getActiveDocumentMetadata()`** — document-generic getters replacing the `getActivePdf*()` trio (plus the new metadata getter).
- `LlmDataPayload` gains a `kind: 'document'` variant (non-PDF documents; PDFs keep `kind: 'pdf'`).
- `documentData` sandbox global — raw bytes of the active document (same value as the legacy `pdfData`); Excel-document formulas can parse it with `XLSX.read(documentData)`.
- `detectDocumentFormat(buffer)` export — magic-byte sniffing (`'pdf' | 'xlsx' | null`); `DocumentFormat`, `DocumentMetadata`, `DocumentExtraction` types exported.
- **Local (in-browser) Excel extraction** — pass your app's SheetJS module via `xlsxModule` (`new ProxyClient(url, token, { xlsxModule: XLSX })` / `LocalProxyConfig.xlsxModule`) and `extractDocument()` parses `.xlsx` entirely on the device — the workbook never reaches any server, and standalone `LocalProxy` extraction works with no proxy at all. Output (pipe rows, `## Page N — "Sheet"` headers, sheet-name-aware search) mirrors the server-side extractor; cells render as their **Excel display strings** (currency, %, date formats), merged banners are de-duplicated to their anchor cell, and in-cell newlines/pipes are sanitized so one row stays one line. Core takes **no** dependency on SheetJS — the module is injected (`extractXlsxLocally` / `XlsxModuleLike` are also exported). Without `xlsxModule`, `ProxyClient` falls back to the proxy's `/common/extract-document`.
...
### Deprecated
- `Proxy.extractPdf()`, `LocalAssistant.addPdfDataset()`, `getActivePdfBuffer()`, `getActivePdfExtractedText()`, `getActivePdfPageCount()` — kept as thin aliases of the document-generic API; no behavior change.
