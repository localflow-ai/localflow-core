...
- \`parseMoney(s)\`: use for any monetary column (debit, credit, balance, amount, price, total). Understands ALL occidental formats — "1 234,56", "1.234,56", "1'234.56", "1,234.56" — plus currency symbols/codes on either side (€, $, EUR 1 234,56, 1,234.56 USD) and accounting negatives "(1 234,56)". Returns NaN for non-monetary strings (account/reference numbers, codes with '/', percentages, descriptions) — safe to call on every column value.
- \`parseNum(s)\`: use for other numeric columns (quantities, prices, percentages, counts). Same universal occidental format handling; percent signs are stripped ("2,23 %" → 2.23 — the value stays as displayed, scale it yourself if needed) and a trailing unit code is ignored.
